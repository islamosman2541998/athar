...
<article class="card" data-reveal><img src="{{ $img !== 'attachments/no_image/no_image.png' ? asset($img) : asset('site/images/athar-devices.png') }}" alt="{{ $trans->title ?? __('athar.fallback.blog') }}" style="height:250px;width:100%;object-fit:cover" loading="lazy"><div class="card-body"><span class="tag">{{ $blog->created_at?->format('Y.m.d') }}</span><h3>{{ $trans->title ?? __('athar.fallback.blog') }}</h3><p>{{ \Illuminate\Support\Str::limit(strip_tags($trans->description ?? ''),145) }}</p><a class="card-link" href="{{ route('site.site.blogs.show',$blog) }}">{{ __('athar.more') }}</a></div></article>
